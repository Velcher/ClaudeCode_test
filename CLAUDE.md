...
- **语音无法区分大小写**：用户说的指令名/缩写无法指定大小写，需根据上下文自动修正。如表格中汇编指令统一用小写（`ld1d` 而非 `LD1D`），正文中专有名词按拼写规则处理
- **专有名词拼写**：`FMoPA` 只能有三种写法 — `fmopa`（全小写）、`Fmopa`（首字母大写）、`FMOPA`（全大写），根据行文语境选择，不可混用大小写如 FMoPA
