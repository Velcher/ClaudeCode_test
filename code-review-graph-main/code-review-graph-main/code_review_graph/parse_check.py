...
def generate_report(target_dir, output_path=None):
    ...
    # Don't print — caller decides.  Avoid Windows GBK emoji crash.

    return results, report


def run_quick_check(repo_root):
    """静默扫描 repo_root 下的 Verilog 文件，返回摘要数据。

    仅扫描 Verilog 文件，不打印报告。完整报告写入
    ``.code-review-graph/parse_report.txt``。build 命令自动调用此函数。
    """
    root = Path(repo_root)
    results, _ = generate_report(root, output_path=root / ".code-review-graph" / "parse_report.txt")

    if results is None:
        return {"verilog_files": 0, "ok": 0, "warn": 0, "bad": 0, "crashed": 0}

    ok = sum(1 for r in results if 0 <= r["error_rate"] < 0.5)
    warn = sum(1 for r in results if 0.5 <= r["error_rate"] < 5.0)
    bad = sum(1 for r in results if r["error_rate"] >= 5.0)
    crashed = sum(1 for r in results if r["error_rate"] < 0)
    return {
        "verilog_files": len(results),
        "ok": ok,
        "warn": warn,
        "bad": bad,
        "crashed": crashed,
